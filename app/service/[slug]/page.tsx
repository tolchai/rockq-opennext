...
export async function generateStaticParams() {
  ...
  try {
    ...
    const paths = (data?.services?.nodes ?? []).map(
      (service: { slug: string }) => ({
        slug: service.slug,
      }),
    );
    ...
    console.error('Error in generateStaticParams [service/slug]:', e);
    return [];
  }
}
...
export async function generateMetadata({ params }: { params: Promise<P> }) {
  ...
  const data = await fetchServiceByUri(slug ? slug : '');

  if (!data.service) {
    return {};
  }

  const { service: page, options } = data;
  return {
...
export default async function Service({ params }: { params: Promise<P> }) {
  const { slug } = await params;
